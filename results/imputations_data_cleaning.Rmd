...
```{r}
nrow(subset(data_imput,FFD_action=="impute"&!is.na(FFD))) #168 cases imputed
```
```{r}
nrow(subset(data_imput,FFD_action=="impute"&is.na(FFD)) )
#11 cases could not be imputed because no info on bud sizes - What to do?
```

Predict shoot volume with number of flowers?

```{r}
nrow(subset(data_imput,data==1&is.na(shoot_vol))) #40 pls with no shoot_vol
nrow(subset(data_imput,data==1&is.na(cum_n_fl))) #31 pls with no cum_n_fl
nrow(subset(data_imput,data==1&is.na(cum_n_fl)&is.na(shoot_vol))) 
#4 pls with no shoot_vol and no cum_n_fl
...
```{r}
nrow(subset(data_imput,data==1&shoot_vol==0)) #1 pl with shoot_vol=0, impute?
```
