...
\newpage
Comparisons of mean temperatures for each year for both stations: they look quite similar
...
```{r detach plyr 1, include=FALSE}
detach(package: plyr)
```


Temperature: average mean, min and max of the two stations for further use + join with precipitation data
...
```{r Define periods }
weather<-merge(weather,unique(alldata[c(1,8)])) #Add column with date of vernal equinox
...
```{r load plyr 2, include=FALSE}
library(plyr)
```

Calculate temperature, precipitation and GDD/GDH for period "b" (from vernal equinox to 60 days
after) and merge with previous data

```{r Calculate temperature, precipitation and GDD/GDH for period "b" and merge with previous data }
mean_weather1_b<-join_all(list(
...
mean_weather3<-merge(merge(mean_weather2,mean_weather2_b),
               aggregate(FFD ~ year, data= alldata, FUN=mean)) #Merge with previous data & mean FFD per year
pander(head(mean_weather3), split.table = 100, style = 'rmarkdown')
```

Models of FFD against temperature, precipitation and GDD/GDH

```{r Models FFD against temperature, precipitation and GDD/GDH}
#List of variables to test as predictors of FFD
varlist<-names(mean_weather3)[c(7:9,19:21,31:33,43:45,55:57,67:69,79:81,91:93,
                                103:105,115:117,127:129,139:141,146:217)]

#Fit univariate linear models of FFD against each predictor
models<-lapply(varlist, function(x) {
  summary(lm(substitute(FFD ~ scale(i), list(i = as.name(x))), data = mean_weather3, na.action=na.exclude))
})

#Build a table with estimate, p and r square for all fitted models
models<-cbind(varlist,
              ldply(models, function(x) coef(x)[2]),
              ldply(models, function(x) coef(x)[8]),
              ldply(models, function(x) x$adj.r.squared)
      )
names(models)<-c("variable","estimate","p","adj.rsquare")
models$sig<-ifelse(models$p<0.05,"*","") # *=p<0.05

#Order models by R square
kable(arrange(models,desc(adj.rsquare)))
```

```{r detach plyr 3, include=FALSE}
detach(package:plyr)
```
...
```{r Plots of best models April-May, echo=FALSE, fig.height=3.5, fig.width=12}
grid.arrange(
ggplot(mean_weather3,aes(x=mean45,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Mean daily temperature April and May")+ylab("Mean first flowering date")+geom_text(x=18,y=70,label="R2=0.76"),
ggplot(mean_weather3,aes(x=GDD3_45,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Sum of GDD3 April and May")+ylab("Mean first flowering date")+geom_text(x=375,y=70,label="R2=0.72"),
ggplot(mean_weather3,aes(x=GDH3_45,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Sum of GDH3 April and May")+ylab("Mean first flowering date")+geom_text(x=9000,y=70,label="R2=0.71"),ncol=3)
```
```{r Plots of best models from vernal equinox to 60 days after, echo=FALSE, fig.height=3.5, fig.width=12}
grid.arrange(
ggplot(mean_weather3,aes(x=mean_b,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Mean daily temperature from vernal equinox to 60 days after")+ylab("Mean first flowering date")+
  geom_text(x=8,y=70,label="R2=0.59"),
ggplot(mean_weather3,aes(x=GDD3_b,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Sum of GDD3 from vernal equinox to 60 days after")+ylab("Mean first flowering date")+
  geom_text(x=325,y=70,label="R2=0.58"),
ggplot(mean_weather3,aes(x=GDH3_b,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Sum of GDH3 from vernal equinox to 60 days after")+ylab("Mean first flowering date")+
  geom_text(x=7750,y=70,label="R2=0.57"),ncol=3)
```

```{r Plots of best models precipitation January-March, echo=FALSE, fig.height=3, fig.width=12}
grid.arrange(
  ggplot(mean_weather3,aes(x=prec123,y=FFD))+geom_point(size=2)+geom_smooth(method="lm")+
  xlab("Sum of precipitation January-March")+ylab("Mean first flowering date")+
  geom_text(x=200,y=70,label="R2=0.17"),
  ggplot(mean_weather3,aes(x=prec123,y=FFD))+theme_void(),
  ggplot(mean_weather3,aes(x=prec123,y=FFD))+theme_void(),ncol=3)
```

Models of FFD against temperatureGDD/GDH AND precipitation 

Precipitation is significant in some cases but R2 does not increase much from the previous best model (with mean45 = mean temperature April and May, R2=0.76)

```{r temperature and precipitation}
summary(lm(FFD~mean45*prec123,mean_weather3))  #Interaction NS, precipitation NS
summary(lm(FFD~mean45+prec123,mean_weather3))  #No interaction, precipitation NS
summary(lm(FFD~GDD3_45*prec123,mean_weather3)) #Interaction NS, precipitation NS
summary(lm(FFD~GDD3_45+prec123,mean_weather3)) #No interaction, precipitation NS (p=0.054)
summary(lm(FFD~GDH3_45*prec123,mean_weather3)) #Interaction NS, precipitation NS
summary(lm(FFD~GDH3_45+prec123,mean_weather3)) #No interaction, precipitation *
summary(lm(FFD~max45+prec123,mean_weather3))   #Precipitation*
summary(lm(FFD~min45+prec123,mean_weather3))   #Precipitation NS
summary(lm(FFD~GDH5_45+prec123,mean_weather3)) #Precipitation*
summary(lm(FFD~GDD5_45+prec123,mean_weather3)) #Precipitation*
```
...
Calculate cumulated GDD/GDH = sum of GDD/GDH until each date, with 3 different starting dates:  
...
weather<-as.data.frame(weather %>% 
         group_by(year) %>%
         mutate(cumGDD3=cumsum(x = GDD3),cumGDD5=cumsum(x = GDD5),
...
weather_vernal<-as.data.frame(subset(weather,period=="b"|period=="c") %>% 
                group_by(year) %>%
                mutate(cumGDD3v=cumsum(x = GDD3),cumGDD5v=cumsum(x = GDD5),
...
weather_apr15<-as.data.frame(subset(weather,date>=vernal+26) %>% 
                group_by(year) %>%
                mutate(cumGDD3a=cumsum(x = GDD3),cumGDD5a=cumsum(x = GDD5),
...
subset(alldata_weather_subs,is.na(mean)&!is.na(FFD)) #No rows with missing weather data
```
...
library(dplyr) #problems with plyr and dplyr

alldata_agg<-as.data.frame(alldata_agg %>% 
                group_by(year) %>%
                mutate(n_cum_FFD = cumsum(x = FFD)))
...
models1<-lapply(names(alldata_agg)[2:25], 
                function(x) {glm(substitute(prop_fl ~ scale(i), list(i = as.name(x))), family=binomial, data = alldata_agg)})

models1_summary<-lapply(X = models1, FUN = summary)
models1_R2<-lapply(X = models1, FUN = NagelkerkeR2)

#Build a table with estimate, p and r square for all fitted models
models1_table<-cbind(names(alldata_agg)[2:25],
              ldply(models1_summary, function(x) coef(x)[2]),
              ldply(models1_summary, function(x) coef(x)[8]),
              ldply(models1_R2, function(x) x$R2)
      )
names(models1_table)<-c("variable","estimate","p","rsquare")
models1_table$sig<-ifelse(models1_table$p<0.05,"*","") # *=p<0.05

#Order models by R square
kable(arrange(models1_table,desc(rsquare)))
```
...
```{r Plots of proportion of plants starting flowering against cumulated GDD/GDH 1, echo=FALSE, fig.height=3, fig.width=11}
...
```{r Plots of proportion of plants starting flowering against cumulated GDD/GDH 2, echo=FALSE, fig.height=3, fig.width=11}
...
```{r 1990, echo=FALSE}
ggplot(weather,aes(date_julian,mean))+geom_line()+facet_wrap(~year,ncol=6)+ggtitle("Mean temperatures for all years")+
...
ggplot(weather,aes(date_julian,GDD3,color=year))+geom_smooth(method="loess",se=F)+xlab("Date (number of days since vernal equinox)")+
...
```{r load plyr 4, include=FALSE}
library(plyr)
```

```{r Calculations winter}
...
```{r Models FFD against winter variables}
#Fit univariate linear models of FFD against each predictor
models2<-lapply(names(mean_weather4)[c(219:228)], function(x) {
  summary(lm(substitute(FFD ~ scale(i), list(i = as.name(x))), data = mean_weather4, na.action=na.exclude))
})

#Build a table with estimate, p and r square for all fitted models
models2<-cbind(names(mean_weather4)[c(219:228)],
              ldply(models2, function(x) coef(x)[2]),
              ldply(models2, function(x) coef(x)[8]),
              ldply(models2, function(x) x$adj.r.square)
      )
names(models2)<-c("variable","estimate","p","adj.rsquare")
models2$sig<-ifelse(models2$p<0.05,"*","") # *=p<0.05

#Order models by R square
kable(arrange(models2,desc(adj.rsquare)))
```
...
models3<-with(alldata_agg,
            by(alldata_agg, year,
               function(x) glm(prop_fl ~ cumGDH3v, data = x,family=binomial)))
coefs_models3<-as.data.frame(sapply(models3, coef)[2,])
coefs_models3$year<-row.names(coefs_models3)
names(coefs_models3)<-c("resp_cumGDH3v","year")

mean_weather5<-merge(mean_weather4,coefs_models3)

#Fit univariate linear models of resp_cumGDH3v against each winter predictor
models4<-lapply(names(mean_weather5)[c(219:228)], function(x) {
  summary(lm(substitute(resp_cumGDH3v ~ scale(i), list(i = as.name(x))), data = mean_weather5, na.action=na.exclude))
})

#Build a table with estimate, p and r square for all fitted models
models4<-cbind(names(mean_weather5)[c(219:228)],
              ldply(models4, function(x) coef(x)[2]),
              ldply(models4, function(x) coef(x)[8]),
              ldply(models4, function(x) x$adj.r.square)
      )
names(models4)<-c("variable","estimate","p","adj.rsquare")
models4$sig<-ifelse(models4$p<0.05,"*","") # *=p<0.05

#Order models by R square
kable(arrange(models4,desc(adj.rsquare)))
```
...
Another ways of testing this relation among winter conditions and response to spring temperature: Models with effects of mean temperature April and May, measures of chilling and their interaction on mean FFD.

```{r influence winter temperature on response to spring temperature}
#Fit linear models of FFD against mean45*chilling measure
models5<-lapply(names(mean_weather5)[c(219:228)], function(x) {
  summary(lm(substitute(FFD ~ scale(mean45)*scale(i), list(i = as.name(x))), 
          data = mean_weather5, na.action=na.exclude))
})

#Build a table with estimate, p and r square for all fitted models
models5<-cbind(names(mean_weather5)[c(219:228)],
              ldply(models5, function(x) coef(x)[4]),
              ldply(models5, function(x) coef(x)[16]),
              ldply(models5, function(x) x$adj.r.square)
      )
names(models5)<-c("variable","estimate","p","adj.rsquare")
models5$sig<-ifelse(models5$p<0.05,"*","") # *=p<0.05

#Order models by R square
kable(arrange(models5,desc(adj.rsquare)))
```

Test this within years instead of among years?

```{r detach plyr 5, include=FALSE}
detach(package:plyr)    
```
