...
# Models of FFD_first, FFD_last, date_10-20-80-90, days_90_10 against weather variables
...
ggplot(data_sel,aes(x=FFD_std,y=n_intact_seeds_rel,color=year))+
  ...
  xlab("Standardized first flowering day")+ylab("Relative number of intact seeds")+
  theme_base()+theme(plot.background=element_rect(fill="white", colour=NA))+my_theme()+
...
# Fig. 1: Among-year variation

```{r Fig. 1 Among-year variation, echo=FALSE, fig.height=18, fig.width=12, message=FALSE}
fig1<-grid.arrange(
# Spring temperature
ggplot(summarySE(weather_45, measurevar="mean", groupvars=c("year")),aes(x=as.factor(year),y=mean))+geom_point()+
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.3)+geom_line()+geom_point(size=3)+
  xlab(NULL)+ylab("Mean daily temperature April and May")+ggtitle("A)")+my_theme(),
# FFD
ggplot(summarySE(data_sel, measurevar="FFD", groupvars=c("year")),aes(x=as.factor(year),y=FFD))+geom_point()+
  geom_errorbar(aes(ymin=FFD-se, ymax=FFD+se), width=.3)+geom_line()+geom_point(size=3)+
  xlab(NULL)+ylab("First flowering day")+ggtitle("B)")+my_theme(),
# Linear selection gradients for FFD
ggplot(subset(selgrads_FFD,term=="FFD_std"), aes(x=year, y=estimate,colour=term,shape=sig))+ 
    geom_errorbar(aes(ymin=estimate-std.error, ymax=estimate+std.error), width=.3,position=position_dodge(0.5))+
    geom_line(position=position_dodge(0.5))+geom_point(size=3,position=position_dodge(0.5))+
    scale_colour_manual(values = c("Black", "#999999"))+scale_shape_manual(values=c(1,19))+
    xlab("Year")+ylab("Selection gradient for first flowering day")+
    geom_hline(yintercept=0,size=0.5,colour="grey",linetype="dashed")+ggtitle("C)")+my_theme(),
ncol=1)
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig1.tiff",
       plot=fig1,device="tiff",width=25,height=35,units="cm",dpi=600)

```

# Fig. 2: Response of FFD to spring temperature

```{r Fig. 1 Response of FFD to spring temperature, echo=FALSE, fig.height=6, fig.width=14}
fig2<-grid.arrange(
ggplot(mean_weather7)+ggtitle("A)")+
  geom_point(aes(x=mean_45,y=date_10),size=2,shape=24)+
  geom_smooth(aes(x=mean_45,y=date_10),method="lm",color="black",lty="dashed",se=F)+
  geom_point(aes(x=mean_45,y=FFD_mean),size=2,shape=19)+
  geom_smooth(aes(x=mean_45,y=FFD_mean),method="lm",color="black",se=F)+
  geom_point(aes(x=mean_45,y=date_90),size=2,shape=8)+
  ...
  theme(plot.margin = unit(c(0,0.5,0,0.5), "cm"))+theme(plot.title=element_text(hjust=-0.15,vjust=-5)),
ggplot(mean_weather7,aes(x=mean_4,y=days_90_10))+ggtitle("B)")+
  ...
  theme(plot.margin = unit(c(0,0.5,0,0.5), "cm"))+theme(plot.title = element_text(hjust =-0.17,vjust=-5)),
ncol=2)
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig2.tiff",
       plot=fig2,device="tiff",width=25,height=11,units="cm",dpi=600)
```

# Fig. 3: Response of selection gradients to spring temperature, position and duration of flowering season

```{r Fig. 3 Response of selection gradients to spring temperature, position and duration of flowering season, fig.height=4, fig.width=14, echo=FALSE}
...
ggplot(mean_weather8,aes(x=mean_45,y=selgrad_FFD))+ggtitle("A)")+
  geom_point(size=2)+geom_smooth(method="lm",color="black",se=F)+
  geom_hline(yintercept=0,size=0.5,colour="grey",linetype="dashed")+
  xlab("Mean daily temperature April and May")+ylab(NULL)+my_theme()+
  theme(plot.title=element_text(hjust=-0.13,vjust=-5))+theme(plot.margin = unit(c(0,0.05,0,0.05), "cm")),
ggplot(mean_weather8)+ggtitle("B)")+
  geom_point(aes(x=date_10,y=selgrad_FFD),size=2,shape=24)+
  geom_smooth(aes(x=date_10,y=selgrad_FFD),method="lm",color="black",lty="dashed",se=F)+
  geom_point(aes(x=FFD_mean,y=selgrad_FFD),size=2,shape=19)+
  geom_smooth(aes(x=FFD_mean,y=selgrad_FFD),method="lm",color="black",se=F)+
  geom_point(aes(x=date_90,y=selgrad_FFD),size=2,shape=8)+
  geom_smooth(aes(x=date_90,y=selgrad_FFD),method="lm",color="black",lty="dotted",se=F)+
    geom_hline(yintercept=0,size=0.5,colour="grey",linetype="dashed")+
  xlab("Day from vernal equinox")+ylab(NULL)+my_theme()+theme(axis.text.y=element_text(color="white"))+
  theme(plot.title=element_text(hjust=-0.12,vjust=-5))+theme(plot.margin = unit(c(0,0.05,0,0.05), "cm")),
ggplot(mean_weather8,aes(x=days_90_10,y=selgrad_FFD))+ggtitle("C)")+
  geom_point(size=2)+geom_smooth(method="lm",color="black",se=F)+
  geom_hline(yintercept=0,size=0.5,colour="grey",linetype="dashed")+
  xlab("Duration of flowering season")+ylab(NULL)+my_theme()+theme(axis.text.y=element_text(color="white"))+
  theme(plot.title=element_text(hjust=-0.12,vjust=-5))+theme(plot.margin = unit(c(0,0.05,0,0.05), "cm")),
ncol=3,left=textGrob("Selection gradient\nfor first flowering day", gp=gpar(fontsize=15,fontfamily="serif"),rot = 90))
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig3.tiff",
       plot=fig3,device="tiff",width=31,height=10,units="cm",dpi=600)
```

# Results 1: Response of position and duration of flowering to temperature

## Position

```{r FFD_mean~temp}
tidy(lm(FFD_mean~mean_45,data=mean_weather8))
glance(lm(FFD_mean~mean_45,data=mean_weather8))$adj.r.squared # Rsquare
predict(lm(FFD_mean~mean_45,data=mean_weather8),
        data.frame(mean_45=with(mean_weather8,min(mean_45)))) # Coldest
predict(lm(FFD_mean~mean_45,data=mean_weather8),
        data.frame(mean_45=with(mean_weather8,max(mean_45)))) # Warmest
```

```{r date_10~temp}
tidy(lm(date_10~mean_45,data=mean_weather8))
glance(lm(date_10~mean_45,data=mean_weather8))$adj.r.squared # Rsquare
predict(lm(date_10~mean_45,data=mean_weather8),
        data.frame(mean_45=with(mean_weather8,min(mean_45)))) # Coldest
predict(lm(date_10~mean_45,data=mean_weather8),
        data.frame(mean_45=with(mean_weather8,max(mean_45)))) # Warmest
```
...
```{r date_90~temp}
tidy(lm(date_90~mean_45,data=mean_weather8))
glance(lm(date_90~mean_45,data=mean_weather8))$adj.r.squared # Rsquare
predict(lm(date_90~mean_45,data=mean_weather8),
        data.frame(mean_45=with(mean_weather8,min(mean_45)))) # Coldest
predict(lm(date_90~mean_45,data=mean_weather8),
        data.frame(mean_45=with(mean_weather8,max(mean_45)))) # Warmest
```
...
```{r days_90_10~temp}
tidy(lm(days_90_10~mean_4,data=mean_weather8))
glance(lm(days_90_10~mean_4,data=mean_weather8))$adj.r.squared # Rsquare
predict(lm(days_90_10~mean_4,data=mean_weather8),
        data.frame(mean_4=with(mean_weather8,min(mean_4)))) # Coldest
predict(lm(days_90_10~mean_4,data=mean_weather8),
        data.frame(mean_4=with(mean_weather8,max(mean_4)))) # Warmest
```

# Results 2: Differences in selection among years
...
Anova(lm(n_intact_seeds_rel ~ FFD_std+FFD_std:year+n_fl_std,data = data_sel),type="II")
#Direct selection for early flowering differs among years
```

# Results 3: Are differences in selection among years related to climatic conditions?
...
data_sel<-merge(data_sel,mean_weather8[c(1,158,218,236,239,240)])
