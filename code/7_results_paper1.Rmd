...
alldata<-read.csv(
  "C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/alldata.csv",
                       header=T,sep="\t",dec=".") #Read field data
temperature<-read.table(
  "C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/temperature.csv",
                        header=T,sep="\t",dec=".") 
precipitation<-read.table(
  "C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/precipitation.csv",
                          header=T,sep="\t",dec=".")
precipitation_extra<-read.table(
  "C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/precipitation_extra.csv",
                          header=T,sep="\t",dec=".")
dates<-read.table(
  "C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/dates.csv",
                        header=T,sep="\t",dec=".") 
...
models_mean<-temperature_wide%>%
  nest(data=-month)%>%
  mutate(
    fit=map(data, ~ lm(mean_S ~ mean_O, data = .x)),
    tidied = map(fit, tidy)
  ) %>% 
  unnest(tidied)%>%
  select(month,term,estimate)%>%
  pivot_wider(names_from=term,values_from=estimate)%>%
  dplyr::rename(intercept=`(Intercept)`,estimate=mean_O)
models_mean

# Models min_S vs mi_O for each month
models_min<-temperature_wide%>%
  nest(data=-month)%>%
  mutate(
    fit=map(data, ~ lm(min_S ~ min_O, data = .x)),
    tidied = map(fit, tidy)
  ) %>% 
  unnest(tidied)%>%
  select(month,term,estimate)%>%
  pivot_wider(names_from=term,values_from=estimate)%>%
  dplyr::rename(intercept=`(Intercept)`,estimate=min_O)
models_min

# Models max_S vs max_O for each month
models_max<-temperature_wide%>%
  nest(data=-month)%>%
  mutate(
    fit=map(data, ~ lm(max_S ~ max_O, data = .x)),
    tidied = map(fit, tidy)
  ) %>% 
  unnest(tidied)%>%
  select(month,term,estimate)%>%
  pivot_wider(names_from=term,values_from=estimate)%>%
  dplyr::rename(intercept=`(Intercept)`,estimate=max_O)
models_max
...
weather$GDD3<-ifelse(with(weather,((max+min)/2)-3)<0,0,
                     with(weather,((max+min)/2)-3))
weather$GDD5<-ifelse(with(weather,((max+min)/2)-5)<0,0,
                     with(weather,((max+min)/2)-5))
weather$GDD7<-ifelse(with(weather,((max+min)/2)-7)<0,0,
                     with(weather,((max+min)/2)-7))
weather$GDD10<-ifelse(with(weather,((max+min)/2)-10)<0,0,
                      with(weather,((max+min)/2)-10))
weather$GDH3<-ifelse(
  with(weather,max<=3),0,
  ifelse(with(weather,max>3&min>3),
         with(weather,24*(min-3)+12*(max-min)),
         with(weather,12*(max-3)^2/(max-min))))
weather$GDH5<-ifelse(
  with(weather,max<=5),0,
  ifelse(with(weather,max>5&min>5),
         with(weather,24*(min-5)+12*(max-min)),
         with(weather,12*(max-5)^2/(max-min))))
weather$GDH7<-ifelse(
  with(weather,max<=7),0,
  ifelse(with(weather,max>7&min>7),
         with(weather,24*(min-7)+12*(max-min)),
         with(weather,12*(max-7)^2/(max-min))))
weather$GDH10<-ifelse(
  with(weather,max<=10),0,
  ifelse(with(weather,max>10&min>10),
         with(weather,24*(min-10)+12*(max-min)),
         with(weather,12*(max-10)^2/(max-min))))
pander(head(weather), split.table = 100, style = 'rmarkdown')
```

### Write table with weather for further use

```{r}
weather_GDD<-write_csv(weather,
                       file="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/weather_GDD.csv")
```
...
mean_weather_aprilmay<-plyr::join_all(list(
    aggregate(mean~year, #Mean of mean daily temperature
              data=subset(weather_study,month==4|month==5),FUN=mean), 
    aggregate(min~year, #Mean of min daily temperature
              data=subset(weather_study,month==4|month==5),FUN=mean), 
    aggregate(max~year, #Mean of max daily temperature
              data=subset(weather_study,month==4|month==5),FUN=mean)), 
    by = NULL, type = "left", match="all")
mean_weather_aprilmay<-gather(mean_weather_aprilmay,variable, value,mean,min,max)%>%
               unite(var, variable) %>% 
               spread(var, value)
names(mean_weather_aprilmay)<-c("year","max_45","mean_45","min_45")
...
write.table(alldata_weather,
             file="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/alldata_weather.csv",
             sep="\t",dec=",",col.names=T,row.names=F)
write.table(tomerge,
             file="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/tomerge.csv",
             sep="\t",dec=",",col.names=T,row.names=F)
```

Load new data with some missing values for weather manually substituted in OpenOffice Calc (merging by date of FFD did not work in cases where FFD was imputed, because that FFD did not correspond exactly to a "real" date - I merged it manually with the closest value)

```{r Load new data with some missing values for weather manually substituted in OpenOffice Calc}
alldata_weather_subs<-read.table("C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/alldata_weather_subs.csv",header=T,sep="\t",dec=".")
...
##Plots of the best models Some plots of the best models of proportion of plants that have started flowering against cumulated GDD/GDH
...
models_propfl<-propfl %>% 
  nest_by(year) %>% 
  mutate(model = list(glm(cbind(n_cum_FFD,max_flowering-n_cum_FFD) ~ FFD, 
...
write.table(mean_weather4,file="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/mean_weather.csv",
            sep="\t",dec=".",col.names=T,row.names=F)
```
...
seldiffs_FFD<-data.frame(data_sel %>% nest_by(year) %>% 
  mutate(model = list(lm(n_intact_seeds_rel ~ FFD_std, data = data))) %>%
    summarise(tidy(model)))
...
seldiffs_FFD_q<-data.frame(data_sel %>% nest_by(year) %>% 
  mutate(model = list(lm(n_intact_seeds_rel ~ FFD_std+I(FFD_std^2),
                         data = data))) %>% summarise(tidy(model)))
...
seldiffs_nfl<-data.frame(data_sel %>% nest_by(year) %>% 
  mutate(model = list(lm(n_intact_seeds_rel ~ n_fl_std, data = data))) %>%
    summarise(tidy(model)))
...
seldiffs_nfl_q<-data.frame(data_sel %>% nest_by(year) %>% 
  mutate(model = list(lm(n_intact_seeds_rel ~ n_fl_std+I(n_fl_std^2), 
                         data = data))) %>% summarise(tidy(model)))
...
ggplot(subset(seldiffs,term=="I(FFD_std^2)"|term=="I(n_fl_std^2)"), aes(x=year, y=estimate*2,colour=term,shape=sig))+ 
    ...
    geom_hline(yintercept=0,size=0.5,colour="grey",linetype="dashed")+my_theme(),
ncol=1)
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/seldiffs1.tiff",
...
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/seldiffs2.tiff",
...
plot_seldiffs3<-ggplot(data_sel,aes(x=FFD_std,y=n_intact_seeds_rel))+facet_wrap(~year,ncol=4)+
  ...
  my_theme()
plot_seldiffs3
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/seldiffs3.tiff",
...
plot_seldiffs4<-ggplot(data_sel,aes(x=n_fl_std,y=n_intact_seeds_rel))+facet_wrap(~year,ncol=4)+
  ...
  my_theme()
plot_seldiffs4
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/seldiffs4.tiff",
...
selgrads_FFD<-data.frame(data_sel %>% nest_by(year) %>% 
  mutate(model = list(lm(n_intact_seeds_rel ~ FFD_std+n_fl_std, 
                         data = data))) %>% summarise(tidy(model)))
...
selgrads_FFD_q<-data.frame(data_sel %>% nest_by(year) %>% 
  mutate(model = list(lm(n_intact_seeds_rel ~ FFD_std+I(FFD_std^2)+n_fl_std+I(n_fl_std^2)+FFD_std:n_fl_std, 
                         data = data))) %>% summarise(tidy(model)))
...
plot_selgrads1<-ggplot(subset(selgrads_FFD,term=="FFD_std"), aes(x=year, y=estimate,colour=term,shape=sig))+ 
    ...
    geom_hline(yintercept=0,size=0.5,colour="grey",linetype="dashed")+my_theme()
plot_selgrads1
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/selgrads1.tiff",
...
aes(x=as.integer(as.character(year)),y=mean))+
  ...
  xlab(NULL)+ylab("\nMean daily temperature (ºC)")+ggtitle("A)")+my_theme()+
  scale_x_continuous(breaks=seq(1987,2017,2))+scale_shape_manual(values=c(21,21,17))+
  scale_fill_manual(values=c("black","white","gray53"))+
  scale_color_manual(values=c("black","black","gray53")),
# FFD
...
ggplot(summarySE(data_sel, measurevar="n_intact_seeds", groupvars=c("year")),
  ...
  scale_x_continuous(breaks=seq(1987,2017,2))+
  xlab(NULL)+ylab("Number of intact seeds\nper individual")+ggtitle("C)")+my_theme(),
# Linear selection gradients for FFD
ggplot(subset(selgrads_FFD,term=="FFD_std"), 
       aes(x=as.integer(as.character(year)), y=estimate))+ 
    geom_errorbar(aes(ymin=estimate-std.error, ymax=estimate+std.error), width=.3,position=position_dodge(0.5))+
    geom_point(size=3,position=position_dodge(0.5),shape=21,aes(fill=sig))+
...
ncol=1)
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/fig1.tiff",
...
figS1<-grid.arrange(
  summarySE(subset(subset(weather,year>1960),month==3),measurevar="min", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=min,color=highlight_flag)) +
  geom_errorbar(aes(ymin=min-se, ymax=min+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("A)")+xlab(NULL)+ylab("Min daily temperature March (ºC)")+my_theme()+
  ...
  summarySE(subset(subset(weather,year>1960),month==4),measurevar="min", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=min,color=highlight_flag)) +
  geom_errorbar(aes(ymin=min-se, ymax=min+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("B)")+xlab(NULL)+ylab("Min daily temperature April (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_y_continuous(labels=function(x) format(x, width=3),breaks=c(-10,-8,-6,-4,-2,0,2,4,6,8))+
  scale_x_continuous(breaks=seq(1961,2017,4)),
  summarySE(subset(subset(weather,year>1960),month==5),measurevar="min", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=min,color=highlight_flag)) +
  geom_errorbar(aes(ymin=min-se, ymax=min+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("C)")+xlab("Year")+ylab("Min daily temperature May (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_y_continuous(labels=function(x) format(x, width=3),breaks=c(-10,-8,-6,-4,-2,0,2,4,6,8))+
  scale_x_continuous(breaks=seq(1961,2017,4)),
...
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/figS1.tiff",
...
figS2<-grid.arrange(
   summarySE(subset(subset(weather,year>1960),month==3),measurevar="mean", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=mean,color=highlight_flag)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("A)")+xlab(NULL)+ylab("Mean daily temperature March (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_y_continuous(labels=function(x) format(x, width=3),breaks=c(-10,-8,-6,-4,-2,0,2,4,6,8))+
  scale_x_continuous(breaks=seq(1961,2017,4)),
  summarySE(subset(subset(weather,year>1960),month==4),measurevar="mean", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=mean,color=highlight_flag)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("B)")+ xlab(NULL)+ylab("Mean daily temperature April (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_y_continuous(labels=function(x) format(x, width=3),breaks=c(-10,-8,-6,-4,-2,0,2,4,6,8))+
  scale_x_continuous(breaks=seq(1961,2017,4)),
  summarySE(subset(subset(weather,year>1960),month==5),measurevar="mean", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=mean,color=highlight_flag)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("C)")+ xlab("Year")+ylab("Mean daily temperature May (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
...
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/figS2.tiff",
...
figS3<-grid.arrange(
  summarySE(subset(subset(weather,year>1960),month==3),measurevar="max", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=max,color=highlight_flag)) +
  geom_errorbar(aes(ymin=max-se, ymax=max+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("A)")+ xlab(NULL)+ylab("Max daily temperature March (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_y_continuous(labels=function(x) format(x, width=3),breaks=c(0,2,4,6,8,10))+
  scale_x_continuous(breaks=seq(1961,2017,4)),
  summarySE(subset(subset(weather,year>1960),month==4),measurevar="max", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=max,color=highlight_flag)) +
  geom_errorbar(aes(ymin=max-se, ymax=max+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("B)")+ xlab(NULL)+ylab("Max daily temperature April (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_y_continuous(labels=function(x) format(x, width=3),breaks=c(0,2,4,6,8,10,12,14,16))+
  scale_x_continuous(breaks=seq(1961,2017,4)),
  summarySE(subset(subset(weather,year>1960),month==5),measurevar="max", groupvars=c("year")) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>% 
  ggplot(aes(x=as.integer(as.character(year)),y=max,color=highlight_flag)) +
  geom_errorbar(aes(ymin=max-se, ymax=max+se), width=.1)+geom_point(size=3)+
  #geom_smooth(method="lm",color="black")+
    scale_color_manual(values = c('grey', 'black'))+
    ggtitle("C)")+ xlab("Year")+ylab("Max daily temperature May (ºC)")+my_theme()+
  theme(plot.title=element_text(hjust=-0.06,vjust=-1))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
...
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/figS3.tiff",
...
figS4<-grid.arrange(
  aggregate(precipitation ~ year,data= subset(weather,month==3),FUN=sum) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>%
  ggplot(aes(x=as.integer(as.character(year)),y=precipitation,fill=highlight_flag))+
  geom_bar(stat="identity",position = "identity")+scale_fill_manual(values = c('grey', 'black'))+
  xlab(NULL)+ylab("Precipitation March (mm)")+ggtitle("A)")+my_theme()+
  ...
  aggregate(precipitation ~ year,data= subset(weather,month==4),FUN=sum) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>%
  ggplot(aes(x=as.integer(as.character(year)),y=precipitation,fill=highlight_flag))+
  geom_bar(stat="identity",position = "identity")+scale_fill_manual(values = c('grey', 'black'))+
  xlab(NULL)+ylab("Precipitation April (mm)")+ggtitle("D)")+my_theme()+
  scale_y_continuous(labels=function(x) format(x, width=3))+
  theme(plot.title=element_text(hjust=-0.07,vjust=0))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_x_continuous(breaks=seq(1945,2017,4)),
  aggregate(precipitation ~ year,data= subset(weather,month==5),FUN=sum) %>% 
  mutate(highlight_flag = ifelse(year>1986&year<1997|year>2005&year<2018, T, F)) %>%
  ggplot(aes(x=as.integer(as.character(year)),y=precipitation,fill=highlight_flag))+
  geom_bar(stat="identity",position = "identity")+scale_fill_manual(values = c('grey', 'black'))+
  xlab("Year")+ylab("Precipitation May (mm)")+ggtitle("C)")+my_theme()+
  scale_y_continuous(labels=function(x) format(x, width=3))+
  theme(plot.title=element_text(hjust=-0.07,vjust=0))+
  theme(plot.margin = unit(c(0.01,0.1,0.01,0.1), "cm"))+
  scale_x_continuous(breaks=seq(1945,2017,4)),
...
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/figS4.tiff",
...
## FFD for each plant (Table 1)

```{r FFD model sel,warning=FALSE,message=FALSE}
# Variables to use
subset1<-data_sel[c(2,3,42,160:162,172:174,184:186,196:198)]
...
sw(modsel_FFD) # Variable importance
...
# Results 4: Are differences in selection among years related to climatic conditions?
...
```{r GLM total}
# Variables to use
subset3<-data_sel[c(3,44:45,160:162,172:174,184:186,196:198)]
...
sw(modsel_total_selection) # Variable importance
...
sw(modsel_direct_selection) # Variable importance
...
## Fig. 3: Response of selection gradients to climate, position and duration of flowering season

```{r Fig. 3 Response of selection gradients to spring temperature, position and duration of flowering season, fig.height=5, fig.width=10, echo=FALSE,warning=FALSE}
subset4<-data_sel[c(3,44:46,160:162,172:174,184:186,196:198)]
...
fig3<-grid.arrange(p1,p2,ncol=2,
                      left=textGrob("Relative number of intact seeds",
                                    ...
                                    gp=gpar(fontsize=16,fontfamily="serif"),rot = 90))
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/fig3.tiff",
       plot=fig3,device="tiff",width=19,height=11,units="cm",dpi=300,compression="lzw")
```
...
data_seldiff<-merge(data_seldiff,unique(data_sel[c(1,185,196:197)]))
...
data_selgrad<-merge(data_selgrad,unique(data_sel[c(1,185,196:197)]))
...
write.table(data_sel,
             file="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/data_sel.csv",
             sep="\t",dec=",",col.names=T,row.names=F)
```

```{r save data, include=FALSE}
save(mean_weather_aprilmay, file="mean_weather_aprilmay.RData")
```

# Analyses revision EL

## Is the rank order of individual FFD relatively consistent across years?

```{r rank FFD}
rank<-data_sel%>%
  group_by(year)%>%
  mutate(rank=rank(FFD))%>%
  select(year,id,rank)%>%
  spread(key=year,value=rank)
cor1<-cor(rank[2:11],use="pairwise.complete.obs")
cor2<-cor(rank[12:23],use="pairwise.complete.obs")
corrplot(cor1, method="number")
corrplot(cor2, method="number")
crossprod(!is.na(rank[2:11])) # Number of observations for each correlation
crossprod(!is.na(rank[12:23])) # Number of observations for each correlation
```

## Hunter et al. 2018

1.  Environmentally-structured fitness function
...
function1<-MCMCglmm(round(n_intact_seeds)~FFD*mean_4,random=~year,rcov =~ units,
                    data=as.data.frame(data_sel),family="poisson",
                    nitt=100000,burnin=10000,thin=10,prior=myprior,verbose=F)
```

2.  Function relating mean trait value to environmental variance

```{r hunter2}
function2<-MCMCglmm(FFD~mean_4,random=~year,rcov=~units,
                    data=as.data.frame(data_sel),family="gaussian",
                    nitt=100000,burnin=10000,thin=10,prior=myprior,verbose=F)
```

3.  Function relating trait variance to environmental variance

```{r hunter3}
#function3<-MCMCglmm()
```

## Does number of flowers / shoot volume differ between the two periods?

```{r nfl 2 periods}
with(data_sel,summary(lm(n_fl~period)))
mean(subset(data_sel,period=="old")$n_fl)
sd(subset(data_sel,period=="old")$n_fl)
mean(subset(data_sel,period=="new")$n_fl)
sd(subset(data_sel,period=="new")$n_fl)
with(data_sel,summary(lm(n_fl~period)))
```
...
```{r FFD-MFD-LFD}
MFD_LFD<-read.delim("C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/MFD_LFD.csv")
MFD_LFD$FFD<-as.Date(as.character(MFD_LFD$FFD),format="%d/%m/%Y")
MFD_LFD$MFD<-as.Date(as.character(MFD_LFD$MFD),format="%d/%m/%Y")
MFD_LFD$LFD<-as.Date(as.character(MFD_LFD$LFD),format="%d/%m/%Y")
MFD_LFD$year<-as.factor(MFD_LFD$year)
MFD_LFD<-data_sel[c(1:3,42,47:56)] %>% inner_join(MFD_LFD[c(1,2,7,8)], by=c("year","id"))
cor(MFD_LFD[c(2,15:16)],use="pairwise.complete.obs")
nrow(subset(MFD_LFD,!is.na(MFD1)))
nrow(subset(MFD_LFD,!is.na(LFD1)))
with(MFD_LFD,summary(lm(MFD1~FFD+n_fl)))
with(MFD_LFD,summary(lm(LFD1~FFD+n_fl)))
with(MFD_LFD,summary(lm(MFD1~FFD)))
with(MFD_LFD,summary(lm(LFD1~FFD)))
cor(MFD_LFD[c(7,13,16)],use="pairwise.complete.obs")
datitos<-MFD_LFD%>%
  group_by(year)%>%
  summarise(mean_LFD1=mean(LFD1,na.rm=T),mean_date_90=mean(date_90,na.rm=T))
cor(datitos[2:3])
```

## How variance in FFD for each id influences mean fitness over all years

```{r var FFD-fitness}
(data_sel_meanfit<-data_sel%>%
  group_by(id)%>%
  summarise(mean_fitness=mean(n_intact_seeds),var_FFD=var(FFD),sd_FFD=sd(FFD)))
summary(lm(mean_fitness~var_FFD,data=data_sel_meanfit))
summary(lm(mean_fitness~sd_FFD,data=data_sel_meanfit))
summary(lm(log(mean_fitness+1)~var_FFD,data=data_sel_meanfit))
summary(lm(log(mean_fitness+1)~sd_FFD,data=data_sel_meanfit))
summary(glm(round(mean_fitness)~var_FFD,data=data_sel_meanfit,family="poisson"))
summary(glm(round(mean_fitness)~sd_FFD,data=data_sel_meanfit,family="poisson"))
summary(glm.nb(round(mean_fitness)~var_FFD,data=data_sel_meanfit))
summary(glm.nb(round(mean_fitness)~sd_FFD,data=data_sel_meanfit))
```

## Effects of climate in previous year

```{r clim previous 1}
weather_study_prev<-subset(subset(weather,year>1985&year<2017),year<1996|year>2004)[1:8]
# Calculations weather by month
# Calculate monthly means of temperature and montly sums of precipitation
mean_weather1_prev<-plyr::join_all(list(
    aggregate(mean~year+month,data=weather_study_prev,FUN=mean), 
    aggregate(min~year+month,data=weather_study_prev,FUN=mean), 
    aggregate(max~year+month,data=weather_study_prev,FUN=mean), 
    aggregate(precipitation~year+month,data=weather_study_prev,FUN=sum)),
    by = NULL, type = "left", match="all")
mean_weather2_prev<-gather(mean_weather1_prev, variable, value,mean,min,max,precipitation) %>%
               unite(var, variable, month) %>% 
               spread(var, value) #Convert to wide format with monthly variables
mean_weather2_prev$year_next<-mean_weather2_prev$year+1
mean_weather2_prev<-mean_weather2_prev[c(7:9,19:21,31:33,43:45,50)]
mean_weather2_prev<-mean_weather2_prev%>%
  rename_at(vars(-year_next),function(x) paste0(x,"_prev"))
mean_weather2_prev$year<-as.factor(mean_weather2_prev$year_next)
mean_weather2_prev$year_next<-NULL
data_sel_prev<-merge(data_sel,mean_weather2_prev)
```
...
```{r clim previous 2}
# Variables to use
subset5<-data_sel_prev[c(2,3,42,206:217)]
subset5[,3:15]<-scale(subset5[,3:15])
globmod_FFD_prev<-lmer(FFD ~ max_3_prev+max_4_prev+max_5_prev+mean_3_prev+mean_4_prev+mean_5_prev+
                         min_3_prev+min_4_prev+min_5_prev+precipitation_3_prev+precipitation_4_prev+
                         precipitation_5_prev+n_fl+(1|id),data = subset5,REML=FALSE,na.action="na.fail")

# Excluding collinear variables with r > 0.5
smat5 <- abs(cor(subset5[, -c(1,2,3)])) <= .5 # TRUE: cor<=0.5,FALSE: cor>0.5
smat5[!lower.tri(smat5)] <- NA

clusterType <- if(length(find.package("snow", quiet = TRUE))) "SOCK" else "PSOCK"
clust1 <- try(makeCluster(getOption("cl.cores", 3), type = clusterType))
clusterExport(clust1, "subset5")
clusterEvalQ(clust1, library(lme4))
modsel_FFD_prev<-pdredge(globmod_FFD_prev,fixed=c("n_fl"),subset=smat5,cluster=clust1)

summary(model.avg(modsel_FFD_prev,subset=delta<2)) # Summary averaged model
sw(modsel_FFD_prev) # Variable importance
r.squaredGLMM(get.models(modsel_FFD_prev,subset=1)$"960") #R square of best model
# Temperature and precipitation of the previous year do affect FFD but the R square is lower (0.30 vs. 0.58)
```
...
```{r clim previous 3}
# Variables to use
subset6<-data_sel_prev[c(3,44:46,206:217)]
subset6[,c(5:16)]<-scale(subset6[,c(5:16)])
globmod_direct_selection_prev<-lmer(n_intact_seeds_rel ~ FFD_std+n_fl_std+
                          FFD_std:max_3_prev+FFD_std:max_4_prev+FFD_std:max_5_prev+
                          FFD_std:mean_3_prev+FFD_std:mean_4_prev+FFD_std:mean_5_prev+
                          FFD_std:min_3_prev+FFD_std:min_4_prev+FFD_std:min_5_prev+
                          FFD_std:precipitation_3_prev+FFD_std:precipitation_4_prev+
                          FFD_std:precipitation_5_prev+(1|id),
                        data = subset6,REML=FALSE,na.action="na.fail")
# Excluding collinear variables with r > 0.5
smat6 <- abs(cor(subset6[, -c(1:4)])) <= .5 # TRUE: cor<=0.5,FALSE: cor>0.5
smat6[!lower.tri(smat6)] <- NA
rownames(smat6)<-paste("FFD_std:", names(smat6[1,1:12]),sep="")
colnames(smat6)<-paste("FFD_std:", names(smat6[1,1:12]),sep="")

clusterType <- if(length(find.package("snow", quiet = TRUE))) "SOCK" else "PSOCK"
clust1 <- try(makeCluster(getOption("cl.cores", 3), type = clusterType))
clusterExport(clust1, "subset6")
clusterEvalQ(clust1, library(lme4))

modsel_direct_selection_prev<-pdredge(globmod_direct_selection_prev,subset=smat6,
                                 fixed=c("FFD_std","n_fl_std"),cluster=clust1)

summary(model.avg(modsel_direct_selection_prev,subset=delta<2)) # Summary averaged model
sw(modsel_direct_selection_prev) # Variable importance
r.squaredGLMM(get.models(modsel_direct_selection_prev,subset=1)$"512") #R square of best model

# Anova (Table 4A) with model including variables that were significant in the averaged model
Anova(lmer(n_intact_seeds_rel ~ FFD_std+n_fl_std+(1|id),data = subset6,REML=FALSE,na.action="na.fail"))
```

## Effects of climate in previous year + current year

### On FFD

```{r clim previuos 4}
# Variables to use
subset7<-data_sel_prev[c(2,3,42,172:174,196:198,209:211,215:217)]
subset7[,3:15]<-scale(subset7[,3:15])
globmod_FFD_prev_curr<-lmer(FFD ~ mean_3_prev+mean_4_prev+mean_5_prev+
                         precipitation_3_prev+precipitation_4_prev+precipitation_5_prev+
                           mean_3+mean_4+mean_5+precipitation_3+precipitation_4+precipitation_5+
                           n_fl+(1|id),data = subset7,REML=FALSE,na.action="na.fail")

# Excluding collinear variables with r > 0.5
smat7 <- abs(cor(subset7[, -c(1,2,3)])) <= .5 # TRUE: cor<=0.5,FALSE: cor>0.5
smat7[!lower.tri(smat7)] <- NA

clusterType <- if(length(find.package("snow", quiet = TRUE))) "SOCK" else "PSOCK"
clust1 <- try(makeCluster(getOption("cl.cores", 3), type = clusterType))
clusterExport(clust1, "subset7")
clusterEvalQ(clust1, library(lme4))
modsel_FFD_prev_curr<-pdredge(globmod_FFD_prev_curr,fixed=c("n_fl"),subset=smat7,cluster=clust1)

summary(model.avg(modsel_FFD_prev_curr,subset=delta<2)) # Summary averaged model
sw(modsel_FFD_prev_curr) # Variable importance
r.squaredGLMM(get.models(modsel_FFD_prev_curr,subset=1)$"127") #R square of best model
# Temperature and precipitation of the previous year do affect FFD but the R square is lower (0.30 vs. 0.58)
```

## Models suggested by Johan (instead of Morrissey)

```{r new models 1}
model1<-lmer(FFD~scale(min_4)+scale(precipitation_3)+scale(precipitation_4)+
               n_fl+(1|id),data = data_sel) #Trait means
data_sel$FFD_ind_mean<-with(data_sel,abs(FFD-FFD_mean))
model2<-lmer(FFD_ind_mean~scale(min_4)+scale(precipitation_3)+
               scale(precipitation_4)+n_fl+(1|id),data = data_sel) #Trait variances
model3<- lmer(n_intact_seeds~scale(min_4)+scale(precipitation_3)+
               scale(precipitation_4)+n_fl+(1|id),data=data_sel) #Mean fitness
model4<- lmer(n_intact_seeds~n_fl+FFD+
                scale(min_4):FFD+scale(precipitation_3):FFD+
                scale(precipitation_4):FFD+n_fl+(1|id),
              data=data_sel) # Unstd trait – absolute fitness relationships
summary(model1)
summary(model2)
summary(model3)
summary(model4)
...
new_data<-merge(merge(data_sel_agg[c(1,146:147,156)],
...
# Analyses second revision EL

## Partial regression plot for model 8 (new Figure 4)
...
ggsave(filename="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/results/figures/fig4_new.tiff",
       plot=fig4_new,device="tiff",width=10,height=10,units="cm",dpi=300,compression="lzw")
...
file="C:/Users/alici/Dropbox/SU/Projects/lathyrus/lathyrus_ms1/data/clean/climate_1961_2017.txt",sep="\t",dec=",",col.names=T,row.names=F)
