...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig3.tiff",
       plot=fig3,device="tiff",width=19,height=9,units="cm",dpi=600)
```
...
### Fig. 4: Response of duration to climate

```{r Fig. 4 Response of duration to spring temperature, echo=FALSE, fig.height=4, fig.width=5,message=FALSE,warning=FALSE}
fig4<-ggplot(mean_weather4)+
...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig4.tiff",
       plot=fig4,device="tiff",width=9,height=8,units="cm",dpi=600)
```
...
effect5<-data.frame(effect(term="days_90_10", 
...
fig5<-grid.arrange(
  ...
  ggplot(effect4, aes(precipitation_4,fit))+ggtitle("D)")+
    ...
    scale_y_continuous(limits=c(1,11),breaks=c(2,4,6,8,10))+
    theme(axis.text.y=element_text(color="white")),
   ggplot(effect5, aes(days_90_10,fit))+ggtitle("E)")+
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5,fill="grey")+
    ...
    scale_y_continuous(limits=c(1,11),breaks=c(2,4,6,8,10))+
    theme(axis.text.y=element_text(color="white")),
  ncol=5,left=textGrob("Number of intact seeds",just="center",hjust=0.48,
                       gp=gpar(fontsize=16,fontfamily="serif"),rot = 90))
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig5.tiff",
       plot=fig5,device="tiff",width=43,height=9,units="cm",dpi=600)
```
...
```{r GLM FFD_mean total}
Anova(lmer(n_intact_seeds_rel ~ FFD_std+FFD_std:FFD_mean+(1|id),data = data_sel),type="II")
#No influences of FFD_mean on selection on FFD
```

```{r GLM days_90_10 total}
Anova(lmer(n_intact_seeds_rel ~ FFD_std+FFD_std:days_90_10+(1|id),data = data_sel),type="II")
#No influences of days_90_10 on selection on FFD
```

## Direct selection 

```{r GLM climate direct}
# Variables to use
...
```{r Fig. 6 Response of selection gradients to spring temperature, position and duration of flowering season, fig.height=5, fig.width=10, echo=FALSE,warning=FALSE}
subset4<-data_sel[c(3,44:46,158:160,170:172,182:184,194:196)]
...
fig6<-grid.arrange(p1,p2,ncol=2,
...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig6.tiff",
       plot=fig6,device="tiff",width=19,height=11,units="cm",dpi=600)
```
