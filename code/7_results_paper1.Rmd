...
alldata<-read.csv(
  "C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/alldata.csv",
                       header=T,sep="\t",dec=".") #Read field data
temperature<-read.table(
  "C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/temperature.csv",
                        header=T,sep="\t",dec=".") 
precipitation<-read.table(
  "C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/precipitation.csv",
                          header=T,sep="\t",dec=".")
precipitation_extra<-read.table(
  "C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/precipitation_extra.csv",
                          header=T,sep="\t",dec=".")
dates<-read.table(
  "C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/dates.csv",
                        header=T,sep="\t",dec=".") 
...
```{r Graph mean temperatures for each year for both stations, echo=FALSE, fig.height=20, fig.width=18}
...
# mean_O=(mean_S-intercept)/estimate  ... (same for min and maz)
...
weather_study<-merge(weather,unique(alldata[c(1,6)])) 
...
write.table(alldata_weather,
             file="C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/alldata_weather.csv",
             sep="\t",dec=",",col.names=T,row.names=F)
write.table(tomerge,
             file="C:/Users/User/Dropbox/SU/Projects/lathyrus/data/clean/tomerge.csv",
             sep="\t",dec=",",col.names=T,row.names=F)
```
...
with(summarySE(data_sel, measurevar="precipitation_13", groupvars=c("year")),tidy(lm(precipitation_13~as.integer(year)))) #NS
...
mean_prec$precipitation_13<-mean_prec$precipitation_1+
  mean_prec$precipitation_2+mean_prec$precipitation_3
...
with(summarySE(mean_prec, measurevar="precipitation_13", groupvars=c("year")),tidy(lm(precipitation_13~as.integer(year)))) #*
...
with(subset(summarySE(mean_prec, measurevar="precipitation_13",groupvars=c("year")),
            year>1986),tidy(lm(precipitation_13~as.integer(year)))) #NS
...
figS4<-grid.arrange(
  ggplot(aggregate(precipitation ~ year,data= subset(weather,month==1|month==2|month==3),FUN=sum))+
    aes(x=as.integer(as.character(year)),y=precipitation)+
  geom_bar(stat="identity",position = "identity")+
    geom_smooth(method="lm",color="black")+
  xlab(NULL)+ylab("Precipitation January-March")+ggtitle("A)")+my_theme()+
...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/figS4.tiff",
...
# Variables to use
subset1<-data_sel[c(2,3,42,158:160,170:172,182:184,195:196,202)]
subset1[,3:15]<-scale(subset1[,3:15])
globmod_FFD<-lmer(FFD ~ max_3+max_4+max_5+mean_3+mean_4+mean_5+min_3+min_4+min_5+
                    precipitation_13+precipitation_4+precipitation_5+n_fl+(1|id),
...
r.squaredGLMM(get.models(modsel_FFD,subset=1)$"1668") #R square of best model
```

## FFD for each plant with year (Table S2)

```{r FFD w year,warning=FALSE,message=FALSE}
summary(lmer(FFD ~ scale(max_5)+scale(min_4)+scale(precipitation_13)+scale(n_fl)+
               as.integer(as.character(year))+(1|id),data = data_sel,
             REML=FALSE,na.action="na.fail"))
r.squaredGLMM(lmer(FFD ~ scale(max_5)+scale(min_4)+scale(precipitation_13)+scale(n_fl)+
               as.integer(as.character(year))+(1|id),data = data_sel,
             REML=FALSE,na.action="na.fail"))
```
...
fig2<-grid.arrange(
  ggplot(data_sel,aes(x=min_4,y=FFD))+ggtitle("A)")+
  ...
  ggplot(data_sel,aes(x=max_5,y=FFD))+ggtitle("B)")+
    ...
    theme(plot.title=element_text(hjust=-0.13,vjust=2)),
  ggplot(data_sel,aes(x=precipitation_13,y=FFD))+ggtitle("C)")+
    geom_point(size=1)+geom_smooth(method="lm",color="black",se=F)+
    xlab("Sum of precipitation January-March")+ylab(NULL)+my_theme()+
    theme(axis.text.y=element_text(color="white"))+
    theme(plot.title=element_text(hjust=-0.13,vjust=2)),
  ncol=3,left=textGrob("First flowering date",just="center",hjust=0.38,
...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig2.tiff",
       plot=fig2,device="tiff",width=28,height=9,units="cm",dpi=600)
```

## Position (Table 1B)

```{r FFD_mean~temp}
mean_weather5$precipitation_13<-mean_weather5$precipitation_1+
  mean_weather5$precipitation_2+mean_weather5$precipitation_3

summary(lm(FFD_mean~scale(min_4)+scale(max_5)+scale(precipitation_13),data=mean_weather5))

## Including year (Table S2)
summary(lm(FFD_mean~scale(min_4)+scale(max_5)+scale(precipitation_13)+
             as.integer(as.character(year)),data=mean_weather5))
```

```{r date_10~temp}
summary(lm(date_10~scale(min_4)+scale(max_5)+scale(precipitation_13),data=mean_weather5))

## Including year (Table S2)
summary(lm(date_10~scale(min_4)+scale(max_5)+scale(precipitation_13)+
             as.integer(as.character(year)),data=mean_weather5))
```

```{r date_90~temp}
summary(lm(date_90~scale(min_4)+scale(max_5)+scale(precipitation_13),data=mean_weather5))

## Including year (Table S2)
summary(lm(date_90~scale(min_4)+scale(max_5)+scale(precipitation_13)+
             as.integer(as.character(year)),data=mean_weather5))
```
...
```{r days_90_10~temp}
summary(lm(days_90_10~scale(min_4)+scale(max_5)+scale(precipitation_13),data=mean_weather5))

## Including year  (Table S2)
summary(lm(days_90_10~scale(min_4)+scale(max_5)+scale(precipitation_13)+
             as.integer(as.character(year)),data=mean_weather5))
```

### Fig. 3: Response of position and duration to climate

```{r Fig. 3 Response of position+duration to spring temperature, echo=FALSE, fig.height=8, fig.width=10,message=FALSE,warning=FALSE}
fig3A<-grid.arrange(
...
```{r Fitness~temp 1}
# Variables to use
subset2<-data_sel[c(3,20,42,158:160,170:172,182:184,195:196,202)]
subset2[,c(3:15)]<-scale(subset2[,c(3:15)])
globmod_fitness<-lmer(n_intact_seeds ~ max_3+max_4+max_5+mean_3+mean_4+mean_5+
                        min_3+min_4+min_5+precipitation_13+precipitation_4+precipitation_5+
...
r.squaredGLMM(get.models(modsel_fitness,subset=1)$"1346") #R square of best model
```
...
```{r Fig 5, echo=FALSE, fig.height=4, fig.width=19}
#Using a model with significant variables in model averaging (effect1-3)
effect1<-data.frame(effect(term="max_4",mod=lmer(n_intact_seeds~precipitation_4+
         max_4+min_5+ n_fl+(1|id),data=data_sel,REML=FALSE,na.action="na.fail"),
         xlevels=list(max_4=seq(7.0,13.6,0.1))))
effect2<-data.frame(effect(term="min_5",mod=lmer(n_intact_seeds~precipitation_4+
         max_4+min_5+ n_fl+(1|id),data=data_sel,REML=FALSE,na.action="na.fail"),
         xlevels=list(min_5=seq(3.9,8.0,0.1))))
effect3<-data.frame(effect(term="precipitation_4",mod=lmer(n_intact_seeds~precipitation_4+
         max_4+min_5+ n_fl+(1|id),data=data_sel,REML=FALSE,na.action="na.fail"),
         xlevels=list(precipitation_4=seq(3.7,79.8,1))))
effect4<-data.frame(effect(term="days_90_10", 
...
fig5<-grid.arrange(
  ...
  ggplot(effect2, aes(min_5,fit))+ggtitle("B)")+
    ...
    scale_y_continuous(limits=c(1,11),breaks=c(2,4,6,8,10))+
    theme(axis.text.y=element_text(color="white")),
  ggplot(effect3, aes(precipitation_4,fit))+ggtitle("C)")+
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5,fill="grey")+
    ...
    scale_y_continuous(limits=c(1,11),breaks=c(2,4,6,8,10))+
    theme(axis.text.y=element_text(color="white")),
   ggplot(effect4, aes(days_90_10,fit))+ggtitle("D)")+
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5,fill="grey")+
    geom_smooth(method="lm",se=T,size=1,color="black",aes(days_90_10,fit))+
    xlab("Duration of flowering season")+ylab(NULL)+my_theme()+
    theme(plot.margin = unit(c(0.07,0.2,0.07,0.2), "cm"))+
    theme(plot.title=element_text(hjust=-0.13,vjust=-4))+
    scale_y_continuous(limits=c(1,11),breaks=c(2,4,6,8,10))+
    theme(axis.text.y=element_text(color="white")),
...
ggsave(filename="C:/Users/User/Dropbox/SU/Projects/lathyrus/results/figures/fig5.tiff",
       plot=fig5,device="tiff",width=38,height=9,units="cm",dpi=600)
```
